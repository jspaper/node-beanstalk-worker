(function() {
  ...
  BeanstalkWorker = (function() {
    ...
    BeanstalkWorker.prototype.find_handler = function(job_type) {
      var handler = eval("this.handlers."+job_type);
      if (handler){
        return handler;
      }
      return null;
    ...
    BeanstalkWorker.prototype.handle_job = function(job_id, job) {
      var handler = this.find_handler(job.type),
          data    = job.data;
      // Compatible with Stalker
      if (typeof(job[0])=="string"){
        handler = this.find_handler(job[0]);
        data=job[1];
        data.job_id=job_id;
        this.log(job[0] +' Job('+job_id.toString()+') : '+JSON.stringify(data));
      }
      if (handler != null) {
        var me=this;
        // Bury job reserves too many time could prevent deadlock.
        var aTmp = me.server.split(':');
        BEANSTALK= new fivebeans.client(aTmp[0], aTmp[1]);
        BEANSTALK.connect(function(err){
          if (err) throw err;
          BEANSTALK.stats_job(job_id, function(err, res){
            if (res.reserves > MAX_RESERVEDS){
              me.log('Buried job : ' + job_id + ' : reserves > '+MAX_RESERVEDS.toString()+' times');
              return me.bury_and_emit_next(job_id);
            }else{
              return me.run_handler_on_job_data(handler, job_id, data);
            }
          });
        });
      } else {
